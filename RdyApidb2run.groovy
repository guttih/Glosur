...
public class ScriptRunner {
     ...
     */
    public static final Pattern delimP = Pattern.compile('^\\s*(--)?\\s*delimiter\\s*=?\\s*([^\\s]+)+\\s*.*\044', Pattern.CASE_INSENSITIVE);
    ...
    public void runScript(Reader reader) throws IOException, SQLException {
        try {
            ...
            try {
                ...
                runScriptPrivate(connection, reader);
            } finally {
     ...
     */
    private void runScriptPrivate(Connection conn, Reader reader) throws IOException,
            SQLException {
    ...
    public static String makePath(String subPath) {
        String bamboo = System.getenv("bamboo_working_directory");
        //bamboo = 'E:/BambooAgents/Agent-2/xml-data/build-dir/KROS-VNTR-JOB1';
        String rootDir;
        if (bamboo != null) {
            rootDir =bamboo.replace("\\","/") + "/ReadyApiTests";
        } else {
            rootDir = 'C:/SVN/Prófanir';
        }
        return rootDir + "/" + subPath;
    }
}
log.info ScriptRunner.Testing()
String SERVER = '10.11.12.13:447';
String DATABASE = 'DBNAME';
String USER = 'THEUSERNAME';
String PASSWORD = 'THEUSERPASSWORD';
Connection connection = DriverManager.getConnection("jdbc:db2://" + SERVER + "/" + DATABASE + ":" + "user=" + USER + ";password=" + PASSWORD + ";");
ScriptRunner runner = new ScriptRunner(connection, true, true)
String file = runner.makePath("Regression/Testgogn/Kortapakkar/Vefthjonustur/CPCWEB001.sql");
